        ...
        try {
            await Promise.all(
                invoices.map((invoice) =>
                    api.delete(`/api/v1/invoices/${invoice.id}`),
                ),
            );

            const message =
                invoices.length === 1
                    ? `Invoice #${invoices[0].id} deleted successfully!`
                    : `${invoices.length} invoices deleted successfully!`;
    ...
    <Dialog.Content class="sm:max-w-[425px]">
        <Dialog.Header>
            <Dialog.Title class="flex items-center gap-2">
                ...
                {invoices.length === 1
                    ? "Delete Invoice"
                    : `Delete ${invoices.length} Invoices`}
            </Dialog.Title>
            <Dialog.Description>
                {#if invoices.length === 1}
                    Are you sure you want to delete Invoice #{invoices[0].id}?
                    This action cannot be undone.
                {:else}
                    Are you sure you want to delete {invoices.length} invoices? This
